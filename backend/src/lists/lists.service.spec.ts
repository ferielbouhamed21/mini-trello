...
describe('ListsService', () => {
  ...
  describe('create', () => {
    it('should create a new list', async () => {
      ...
      const saveMock = jest.fn().mockResolvedValue(mockList);
      jest.spyOn(listModel, 'constructor' as any).mockReturnValue({
        save: saveMock,
      });

      // Test that board exists check works
      await expect(
        service.create(mockBoardId.toString(), createListDto),
